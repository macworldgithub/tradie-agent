...
export class MailService {
  ...
  /** Shared banner attachment â€” embedded as an inline CID image */
  ...
  async sendPostCallSummaryEmail(
    to: string,
    cc: string[] | undefined,
    subject: string,
    data: {
    ...
    const html = this.wrapHtml(`
      <div style="line-height: 1.6; padding: 0 20px 20px;">
        <h1 style="color: #0056b3;">Post-Call Summary</h1>

        <h2 style="color: #444; border-bottom: 2px solid #eee; padding-bottom: 5px;">Customer Booking</h2>
        <table style="width: 100%; border-collapse: collapse; font-size: 15px;">
          <tr><td style="padding: 6px 0; color: #666; width: 160px;">Name</td><td><strong>${data.customerName}</strong></td></tr>
          <tr><td style="padding: 6px 0; color: #666;">Phone</td><td>${data.customerPhone}</td></tr>
          <tr><td style="padding: 6px 0; color: #666;">Address</td><td>${data.address}</td></tr>
          <tr><td style="padding: 6px 0; color: #666;">Urgency</td><td>${data.urgency}</td></tr>
          <tr><td style="padding: 6px 0; color: #666;">Service Type</td><td>${data.serviceType}</td></tr>
          <tr><td style="padding: 6px 0; color: #666;">Problem</td><td>${data.problemDescription}</td></tr>
          <tr><td style="padding: 6px 0; color: #666;">Preferred Time</td><td>${data.preferredTime}</td></tr>
          <tr><td style="padding: 6px 0; color: #666;">Summary</td><td>${data.summary}</td></tr>
        </table>

        <h2 style="color: #444; border-bottom: 2px solid #eee; padding-bottom: 5px; margin-top: 24px;">Call Metadata</h2>
        <table style="width: 100%; border-collapse: collapse; font-size: 15px;">
          <tr><td style="padding: 6px 0; color: #666; width: 160px;">DID Number</td><td>${data.didNumber}</td></tr>
          <tr><td style="padding: 6px 0; color: #666;">Start Time</td><td>${data.startTime}</td></tr>
          <tr><td style="padding: 6px 0; color: #666;">End Time</td><td>${data.endTime}</td></tr>
          <tr><td style="padding: 6px 0; color: #666;">Duration</td><td>${data.duration} seconds</td></tr>
        </table>

        <h2 style="color: #444; border-bottom: 2px solid #eee; padding-bottom: 5px; margin-top: 24px;">Tradie Info</h2>
        <p style="white-space: pre-line;">${data.tradieInfo}</p>
      </div>
    `);
