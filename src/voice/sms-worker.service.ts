...
export class SmsWorkerService {
  ...
  async processPostCallSms(event: CallEndedEvent): Promise<void> {
    const { enfonicaCallId, customerNumber, didNumber, startTime, endTime } =
      event;
    this.logger.log(
      `Starting post-call SMS processing for call ${enfonicaCallId}`,
    );
    ...
    try {
      ...
      if (callRecord.status !== 'completed' || !callRecord.summary) {
        this.logger.log(
          `Skipping post-call SMS for ${enfonicaCallId}: call status is '${callRecord.status}' (not completed)`,
        );
        return;
      }
      ...
      const activeSmsTradies = tradies.filter(
        (t) =>
          t.phoneNumber &&
          (t.notificationPreference === 'sms' ||
            t.notificationPreference === 'both'),
      );

      if (activeSmsTradies.length === 0) {
        this.logger.log(
          'Skipping SMS notification: no active tradies with SMS notification preference',
        );
        return;
      }

      const tradieInfoSection = tradies
        .map((t) => `* name: ${t.name}\n* phone: ${t.phoneNumber}`)
        .join('\n\n');
...
`;
      ...
      for (const tradie of activeSmsTradies) {
        this.logger.log(
          `Sending post-call summary SMS to: ${tradie.phoneNumber}`,
        );
        await this.notificationService.sendSms(tradie.phoneNumber, body);
      }
    } catch (err) {
      this.logger.error(
        `Failed to process post-call SMS for ${enfonicaCallId}:`,
        err,
      );
    }
  }
