...
interface RealtimeSession {
  ...
  // ───────────────────────────────────────────────────────────────────
  // RTP pacer state — queue + interval timer that drains one
  // 20ms slin16 frame at a time to match real-time audio cadence.
  // ───────────────────────────────────────────────────────────────────
  outboundFrameQueue: Buffer[];
  outboundPacerTimer: NodeJS.Timeout | null;

  // ───────────────────────────────────────────────────────────────────
  // slin16 path: only one resampler needed now (inbound: 16kHz → 24kHz).
  // Outbound (ElevenLabs → Asterisk) requires zero conversion because
  // ElevenLabs outputs PCM 16kHz which IS slin16 — sent straight through.
  // ───────────────────────────────────────────────────────────────────
  resampler16to24: SpeexResampler | null; // inbound: Asterisk slin16 → OpenAI 24kHz
}
...
// Each slin16 frame: 16000 Hz * 20ms * 2 bytes (16-bit) = 640 bytes
const SLIN16_FRAME_BYTES = 640;
const SLIN16_FRAME_INTERVAL_MS = 20;
...
export class VoiceService {
  ...
  /**
   * ───────────────────────────────────────────────────────────────────
   * OUTBOUND: ElevenLabs → Asterisk (slin16 path)
   *
   * ElevenLabs is configured with output_format=pcm_16000, which means
   * it already outputs raw 16-bit PCM @ 16kHz — that IS slin16.
   * Zero conversion needed. We just split into 640-byte (20ms) frames
   * and pace them through the RTP pacer at real-time cadence.
   * ───────────────────────────────────────────────────────────────────
   */
  private sendAudioToAri(callId: string, audioDelta: string): void {
    try {
      ...
      // ElevenLabs PCM 16kHz = slin16. Send straight through — no resampling, no ulaw encoding.
      const pcm16kBuffer = Buffer.from(audioDelta, 'base64');
      this.logger.debug(
        `[${callId}] sendAudioToAri: received ${pcm16kBuffer.length} bytes (slin16, no conversion needed)`,
      );

      // Split into 20ms slin16 frames (640 bytes each) and enqueue
      let frameCount = 0;
      for (let i = 0; i < pcm16kBuffer.length; i += SLIN16_FRAME_BYTES) {
        const frame = pcm16kBuffer.subarray(i, i + SLIN16_FRAME_BYTES);
        session.outboundFrameQueue.push(Buffer.from(frame));
        frameCount++;
      }
      ...
      // Kick the pacer if not already running
      this.startOutboundPacer(callId);
    } catch (err) {
      this.logger.error(
        `[${callId}] sendAudioToAri FAILED: ${(err as Error).message}`,
  ...
  /**
   * Starts the per-session RTP pacer if it is not already running.
   * The pacer emits one 20ms slin16 frame per tick to match real-time
   * audio cadence. Auto-stops when the queue empties.
   */
  private startOutboundPacer(callId: string): void {
    const session = this.sessions.get(callId);
    ...
    session.outboundPacerTimer = setInterval(() => {
      ...
      try {
        this.ariRtpMediaService.sendSlin16ToCall(callId, frame);
        //this.ariRtpMediaService.sendUlawToCall(callId, frame);
      } catch (err) {
        this.logger.error(
          `[${callId}] Pacer send failed: ${(err as Error).message}`,
  ...
  /**
   * ───────────────────────────────────────────────────────────────────
   * INBOUND: Asterisk → OpenAI (slin16 path)
   *
   * Asterisk now sends raw 16-bit PCM @ 16kHz (slin16).
   * We only need to upsample 16kHz → 24kHz for OpenAI Realtime API.
   * The old ulaw decode step is gone entirely.
   * ───────────────────────────────────────────────────────────────────
   */
  async sendInboundAudio(sessionId: string, pcm16kPayload: Buffer): Promise<void> {
    const session = this.sessions.get(sessionId);
    if (!session || !session.ws || session.ws.readyState !== WebSocket.OPEN) return;

    try {
      // Asterisk slin16 is already 16-bit PCM @ 16kHz. Just upsample to 24kHz for OpenAI.
      const pcm24k = await this.upsample16kTo24k(sessionId, pcm16kPayload);
      session.ws.send(
        JSON.stringify({
          type: 'input_audio_buffer.append',
          audio: pcm24k.toString('base64'),
  ...
  /**
   * Upsample 16kHz PCM → 24kHz PCM using SpeexResampler.
   * Used for the inbound path (Asterisk slin16 → OpenAI Realtime).
   * Resampler instance is created lazily per session and reused.
   */
  private async upsample16kTo24k(sessionId: string, input: Buffer): Promise<Buffer> {
    const session = this.sessions.get(sessionId);
    if (!session) return Buffer.alloc(0);

    await SpeexResampler.initPromise;

    if (!session.resampler16to24) {
      session.resampler16to24 = new SpeexResampler(1, 16000, 24000, 5);
    }

    return session.resampler16to24.processChunk(input);
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      ws.on('open', () => {
        ...
        this.sessions.set(sessionId, {
          ...
          outboundFrameQueue: [],
          outboundPacerTimer: null,
          resampler16to24: null, // single resampler: inbound 16kHz → 24kHz
        });
  ...
  private openElevenLabsStream(sessionId: string, force = false): void {
    ...
    const apiKey = this.config.get<string>('ELEVENLABS_API_KEY');
    const voiceId = this.config.get<string>('ELEVENLABS_VOICE_ID');
    // output_format=pcm_16000 → raw 16-bit PCM @ 16kHz = slin16, zero conversion needed on receipt
    const wsUrl = `wss://api.elevenlabs.io/v1/text-to-speech/${voiceId}/multi-stream-input?model_id=eleven_flash_v2_5&output_format=pcm_16000&inactivity_timeout=180`;
  ...
  private async handleRealtimeEvent(
    ...
    switch (event.type) {
      ...
      case 'input_audio_buffer.speech_started':
        ...
        // Flush queued outbound frames so the bot stops talking immediately on barge-in
        this.flushOutboundAudio(sessionId);
...
- ALWAYS call save_customer_booking before ending the call. This rule overrides everything else.`;
  ...
  closeSession(sessionId: string): void {
    ...
    if (session) {
      ...
      session.resampler16to24 = null; // cleanup inbound resampler
      this.sessions.delete(sessionId);
      this.logger.log(`[${sessionId}] Active Call Disconnected`);
    }
  }
