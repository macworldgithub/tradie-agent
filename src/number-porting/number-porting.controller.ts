import { Controller, Get, Param, Req, Res, UseGuards, NotFoundException, InternalServerErrorException } from '@nestjs/common';
...
export class NumberPortingController {
  ...
  @Get(':id/document')
  @ApiOperation({ summary: 'Download the supporting document for a porting request' })
  async getDocument(@Param('id') id: string, @Req() req: any, @Res() res: Response) {
    // Get the file path securely via the service
    const documentPath = await this.numberPortingService.getDocumentPath(id, req.user);
    ...
    res.setHeader('Content-Disposition', `inline; filename="supporting-document.pdf"`);
