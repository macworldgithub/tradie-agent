...
export class AriService implements OnModuleInit, OnModuleDestroy {
  ...
  async originateTradieCall(
    tradieNumber: string,
    callerNumber: string,
    didNumber: string,
  ) {
    const endpoint = `${this.getOriginateEndpointPrefix()}${tradieNumber}`;
    this.logger.log(
      `Originating tradie call via ARI endpoint=${endpoint} caller=${callerNumber} did=${didNumber}`,
    );
  ...
  private getOriginateEndpointPrefix(): string {
    return (
      this.configService.get<string>('ASTERISK_ORIGINATE_ENDPOINT_PREFIX') ||
      'PJSIP/'
    );
  }

  private getFallbackEndpointPrefix(): string {
    return (
      this.configService.get<string>('ASTERISK_FALLBACK_ENDPOINT_PREFIX') ||
      this.getOriginateEndpointPrefix()
    );
  }
