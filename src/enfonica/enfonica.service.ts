...
export class EnfonicaService {
  ...
  constructor(
    ...
    @InjectModel(Tradie.name)
    private readonly tradieModel: Model<TradieDocument>,
  ...
  private getRegulatoryListing(countryCode: string): string {
    if (countryCode === 'AU') {
      ...
      if (!listing)
        throw new Error('Missing env var: ENFONICA_REGULATORY_LISTING_ID_AU');
      return listing;
    }
    if (countryCode === 'NZ') {
      const listing = process.env.ENFONICA_REGULATORY_LISTING_ID_NZ;
      if (!listing)
        throw new Error('Missing env var: ENFONICA_REGULATORY_LISTING_ID_NZ');
      return listing;
    }
    throw new Error(
      `Unsupported country code for regulatory listing: ${countryCode}`,
    );
  }
  ...
  async provisionFirstTimeDid(userId: string) {
    ...
    const existingDid = await this.didModel
      .findOne({ companyId: userId })
      .exec();
    if (user.phoneNumberInstanceName || existingDid) {
      this.logger.warn(
        `User ${userId} already has a number provisioned or DID exists. Skipping.`,
      );
      return;
    }
    ...
    if (country === 'AU') {
      ...
      try {
        ...
        this.logger.log(
          `Provisioning AU number: cityCode=${cityCode} → prefix=${prefix}`,
        );
      } catch (error) {
    ...
    const [numbers] =
      await this.phoneNumbersClient.searchPhoneNumbers(searchParams);

    if (!numbers || numbers.length === 0) {
      const location = country === 'AU' ? user.cityName || cityCode : country;
      throw new BadRequestException(
        `No numbers currently available for ${location}, please try again shortly or pick another city`,
      );
    }
    ...
    try {
      const response =
        await this.phoneNumberInstancesClient.createPhoneNumberInstance({
          parent,
          phoneNumberInstance: {
      ...
      phoneNumberString =
        ...
        (selectedNumber as any).phoneNumber;

      if (instance.lifecycleState !== 'ACTIVE') {
        this.logger.warn(
          `Number lifecycleState is not ACTIVE: ${instance.lifecycleState}`,
        );
      }
    } catch (err: any) {
      this.logger.error(
        `Error purchasing Enfonica number for user ${userId}`,
        err.stack || err,
      );
    ...
    try {
      ...
      const incomingCallWebhook =
        this.configService.get<string>('INCOMING_CALL_WEBHOOK') ||
      ...
      if (firstTradie) {
        ...
        this.logger.warn(
          `No tradie found for company ${userId}, DID was not mapped in AdminService`,
        );
      }
      ...
      this.logger.error(
        `FATAL: Enfonica number ${instanceName} was purchased but subsequent setup failed for user ${userId}`,
        error.stack || error,
      );
      throw error;
    }
